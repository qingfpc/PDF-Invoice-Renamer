...
1.  **下载程序**：
    ...
    * 下载 `InvoiceHelper.exe` (或原名的 exe 文件)。

2.  **运行工具**：
    * 双击打开 `InvoiceHelper.exe`。
    * **步骤一**：按照提示输入（或拖入）存放发票的文件夹路径。
    * **步骤二**：选择是否需要重命名。如果需要，输入数字选择格式（例如 `1` 代表 `日期_销售方_金额`）。
    * **步骤三**：选择是否需要合并 PDF。确认后，程序会自动生成一个包含所有发票的 A4 排版 PDF 文件。

3.  **查看结果**：
    * 处理完成后，文件夹内的文件名将变得规范，且会生成一个新的 `发票合集_时间戳.pdf` 文件用于直接打印。
